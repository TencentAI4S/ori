...
from pepdif.rounding import denoised_fn_round
from pepdif.text_datasets import load_data_text

import time
from pepdif.utils import dist_util, logger
...
def main():
    ...
    out_dir = os.path.join(args.out_dir, f"PepDif")
    if not os.path.isdir(out_dir):
        os.mkdir(out_dir)

    out_path = os.path.join(out_dir, f"seed{args.seed2}_step{args.clamp_step}.json")
        ...
        for i in range(world_size):
            if i == rank:
                ...
                for (recov, ref, src) in zip(word_lst_recover, word_lst_ref, word_lst_source):
                    recov = recov.replace(' ', '').replace('[PAD]', '').replace('[END]', '').replace('X', '')
                    src = src.replace(' ', '').replace('[PAD]', '').replace('[END]', '')
