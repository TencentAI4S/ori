...
from pepdif import gaussian_diffusion as gd
from pepdif.gaussian_diffusion import SpacedDiffusion, space_timesteps
from pepdif.transformer_model import TransformerNetModel
...
class myTokenizer():
    ...
    def __init__(self, args):
        if args.vocab == 'bert':
            tokenizer = PreTrainedTokenizerFast(tokenizer_file='pepdif/utils/tokenizer.json', pad_token="[PAD]")
    ...
    def encode_token(self, sentences):
        if isinstance(self.tokenizer, dict):
            ...
            for i, input_id in enumerate(input_ids):
                input_ids[i].append(self.sep_token_id)
        else:
...
def load_defaults_config():
    ...
    """
    with open('pepdif/config.json', 'r') as f:
        return json.load(f)
