...
from fpa import Batch_Prediction, Single_Prediction
from net import ASMNet
...
class FPAExperiment:
    ...
    def _initialize_dataset(self, task, onthefly, num_pos_emb, num_rbf, contact_cutoff):
        if task == 'batch':
            return Batch_Prediction(onthefly=onthefly, num_pos_emb=num_pos_emb, num_rbf=num_rbf, contact_cutoff=contact_cutoff)
        elif task == 'single':
    ...
    def predict_batch(self, checkpoints, save_prediction=False, save_df_name='prediction.tsv'):
        for i, model in enumerate(self.models):
            model.load_state_dict(torch.load(checkpoints[i]), strict=False)
    ...
    def predict_single(self, checkpoints, protein_id='7lll_R', peptide_seq='AELAELAEL'):
        ...
        y_mean = sum(model(prot_data.to(self.devices[0]), pep_data.to(self.devices[0]), smi_data.to(self.devices[0]))[0].cpu().item() for model in self.models) / len(self.models)
