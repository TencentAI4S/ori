...
from tgnn.model.layer import Linear, precompute_freqs_cis, apply_rotary_emb
...
class InvariantPointAttention(nn.Module):
    ...
    def __init__(
        ...
        self.pack_kv = pack_kv if not self.pack_qkv else False
        ...
        if self.pack_qkv:
            ...
            self.linear_q = Linear(self.c_s, self.dim, bias=bias)
            self.linear_k = Linear(self.c_s, self.dim, bias=bias)
            self.linear_v = Linear(self.c_s, self.dim, bias=bias)
            self.linear_q_points = Linear(self.c_s, self.num_heads * self.num_qk_points * 3)
            self.linear_k_points = Linear(self.c_s, self.num_heads * self.num_qk_points * 3)
            self.linear_v_points = Linear(self.c_s, self.num_heads * self.num_v_points * 3)
    ...
    def update_freqs_cis(self, seq_len, device=None):
        if self.freqs_cis is None or seq_len > self.freqs_cis.shape[0]:
            self.freqs_cis = precompute_freqs_cis(seq_len,
                                                  rotary_dim=self.head_dim,
                                                  device=device)

    def _project_qkv(self, s: torch.Tensor, freqs_cis: torch.Tensor = None):
    ...
    def _project_points(self,
        ...
        if self.pack_qkv:
            ...
            qkv_pts = self._apply_points(qkv_pts, r)
            ...
            q_pts = self._apply_points(q_pts, r)
            # [*, seq_len, H * (P_q + P_v) * 3]
            kv_pts = self.linear_kv_points(s)
            # [*, seq_len, H * (P_q + P_v), 3]
            kv_pts = self._apply_points(kv_pts, r)
            # [*, seq_len, H, P_q/P_v, 3]
            ...
            k_pts = self._apply_points(k_pts, r)
            v_pts = self.linear_v_points(s)
            v_pts = self._apply_points(v_pts, r)
    ...
    def forward(
        ...
        if self.rotary_embedding:
            seq_len = s.shape[-2]
            self.update_freqs_cis(seq_len, device=s.device)
        ...
        if mask is not None:
            ...
            square_mask = torch.finfo(attn_weights.dtype).max * (square_mask - 1)
        ...
        # [*, N_res, H * P_v]
        o_pt_norm = flatten_final_dims(o_pt.norm(dim=-1), 2)
        # [*, N_res, H * P_v, 3]
